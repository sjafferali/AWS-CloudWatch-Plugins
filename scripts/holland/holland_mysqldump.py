...
if __name__ == '__main__':
    ...
    if dump_age > 172800:
        result = 1
        print current_date + " - ERROR: holland-plugin: mysqldump file is older than 2d."
    if len(matched_lines) > 1:
        print current_date + " - ERROR: holland-plugin: detected error(s) in the backup log: "+last_error
        result = 1
    if "false" in (str(all(mysql_check_creds)).lower(), str(all(mysql_check_ping)).lower(), str(all(mysql_check_status)).lower()):
        print current_date + " - ERROR: Issue connecting to MySQL."
        result = 1
